...
Paperclip is a local personal ledger. It does not provide runtime secret
...
`PAPERCUT_HOME` remains a deprecated fallback when `PAPERCLIP_HOME` is unset.
...
Paperclip. Use it at your own risk.
...
Paperclip stores what the user gives it, and the user should not give it secrets.
...
If Paperclip later needs stricter secret handling, the preferred direction is to
